...
const Section = ({ title, items, togglePop }) => {
  ...
  <h3 id={title} className="text-3xl my-5">{title}</h3>
  <hr />

  <div className="grid gap-10 grid-cols-5 mx-auto max-w-7xl mt-20">
    {items.map((item, index) => (
      <div className="w-full max-w-xs mx-auto my-10 cursor-pointer relative transition duration-250 ease-in-out shadow-green" key={index} onClick={() => togglePop(item)}>
